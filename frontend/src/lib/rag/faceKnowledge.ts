/**
 * 面相知识检索——根据面相描述检索对应知识库片段
 */
import type { RagSource } from '@/types/consult';

const CORPUS_PATH = 'corpus/mianxiang/mianxiang.txt';
      ...
      if (res.ok) {
        ...
        for (const raw of parts) {
          ...
          if (para.startsWith('### ') || para.startsWith('## ')) {
            currentBook = para.replace(/^#+\s*/, '').split(/[：:。]/)[0].trim() || '面相入门';
            continue;
          }
...
export async function searchFaceKnowledge(
  descriptions: string[],
  keywords: string[],
): Promise<RagSource[]> {
