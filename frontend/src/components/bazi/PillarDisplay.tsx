...
export default function PillarDisplay({ pillar, label }: PillarDisplayProps) {
  return (
    <div className="flex flex-col items-center rounded-lg border border-red-900/40 bg-black/60 px-2 py-2 shadow-lg shadow-red-900/10 backdrop-blur-sm sm:px-4 sm:py-3">
      {/* Label */}
      <span className="mb-1.5 text-[10px] font-medium text-gray-500 uppercase tracking-widest sm:text-xs sm:mb-2">
        {label}
      </span>
      ...
      {/* 纳音 */}
      {pillar.nayin && (
        <span className="mt-2 text-[11px] italic text-gray-500">
          {pillar.nayin}
        </span>
      )}
    </div>
